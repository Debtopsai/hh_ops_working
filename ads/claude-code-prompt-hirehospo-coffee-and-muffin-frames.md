...
| `frames/05-entry-figure.html` | Shot 7 · 0:11.5-0:12.5 | 1.0s | `From $14.99/day` · `FUNDING UP TO $50,000 · PACKAGES FROM $14.99/DAY` · `YOUR PAYMENT DEPENDS ON EQUIPMENT VALUE AND TERM` · `SUBJECT TO CREDIT APPROVAL` | **REUSE** the disclosed-figure component from `50k-from-1499`. Figure scales 0.94 → 1.0 with a flame underline wipe; disclosure fades in at +0.15s **inside the same container** |
| `frames/06-end-card.html` | Shot 8 · 0:12.5-0:15 | 2.5s | `HireHospo` · `Premium kitchen equipment, fully serviced and warranted, on low weekly payments.` · `Get our latest stock list today` · `FULLY SERVICED · WITH WARRANTY · SUBJECT TO CREDIT APPROVAL` · `hirehospo.com` | **REUSE** unchanged |
...
- **Compliance:**
  ...
  - **No GST line appears anywhere** - removed by client direction. Do not reinstate it. **"Subject to credit approval"** in the disclosure and on frames 02 and 06.
