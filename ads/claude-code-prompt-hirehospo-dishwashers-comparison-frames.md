# Claude Code Prompt - HireHospo 16s "Get The Dishwasher You Really Want": Animated Frames

> **This ad is a re-skin.** Open Claude Code with `HireHospo_dishwashers-comparison_script_16s.md`,
> `HireHospo_dishwashers-comparison_storyboard.md`, and **`ad/machines-you-really-want/`**.
...
Produce `ad/dishwashers-comparison/` as a **configuration of the existing SCC101 comparison build**, plus one
...
| Token | Value |
|---|---|
| `--product-name` | `Starline M2` |
| `--product-image` | Starline M2 cut-out, transparent background, front three-quarter, lit upper left |
| `--buy-price` | `$8,730` ⚠ **derived, confirm against the portal brochure before publishing** |
| `--entry-rate` | `$7.86` ⚠ **derived, confirm against the portal brochure before publishing** |
| `--short-figure` | `8.7K` (used in rows 2 and in the payoff strike-through) |
| `--hero-line` | `GET THE DISHWASHER YOU REALLY WANT` |
| `--outcome-1` | `STAFF ON THE FLOOR, NOT AT THE SINK` |
| `--outcome-2` | `NO PILE-UP AT THE PASS` |
| `--category-sfx` | rack clack, wet and close |
...
| 2 | ✗ `8.7K OUT OF YOUR POCKET` | ✓ `KEEP 8.7K IN YOUR POCKET` |
...
- **`8.7K OUT OF YOUR POCKET` on the left, never `8.7K DEBT`** - a cash purchase spends capital, it does not create debt.
