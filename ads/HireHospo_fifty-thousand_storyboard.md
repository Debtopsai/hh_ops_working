...
| 6 | 0:10-0:11.5 | GFX tiles light | The same four category tiles from shot 3 light one at a time - the kitchen happening | `COMBI OVENS` · `COMMERCIAL DISHWASHERS` · `RANGES` · `FOOD PREP` | "Fully serviced..." | Four tile ticks, warm, 150ms apart | Hard cut |
| 7 | 0:11.5-0:12.5 | Serviced badge | Condition badge stamps on over the tiles' lower left | **`FULLY SERVICED · WITH WARRANTY`** *(flame border, stamp settle)* | "...with warranty." | Stamp settle; light metallic ring | Hard cut |
| 8 | 0:12.5-0:13.5 | GFX payment model | The payment model resolves in flame. **No figure - the model, not a number** | **low weekly payments** | "Low weekly payments" | Till/receipt tick (−16dB) | Hard cut |
...
| `frame-serviced-badge` | Shot 7 | (overlay) | `FULLY SERVICED · WITH WARRANTY` | Stamp settle: scale 1.08 → 1.0 in 0.25s | **REUSED** unchanged. This frame's one flame highlight |
