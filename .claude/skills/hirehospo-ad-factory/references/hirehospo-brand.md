...
> brand book, SwipePages landing-page export) and the live site `hirehospo.com`; (2) the HireHospo
> operating skills (`hirehospo-products`, `hirehospo-sales`, `hirehospo-credit`,
> `hirehospo-collections`, `hirehospo-ops`) - the **business facts below are hard constants** drawn
> from them; (3) this file's visual section, which is **provisional** (no official UI kit exists
> yet) - if a kit or brand book appears, it wins and this file should be updated.
...
Category facts and price bands come from `hirehospo-products` (read `references/category-guide.md`
and `data/active-products.csv` there for real product names). Match gear to the audience - a cafe
ad shows an undercounter dishwasher, not a 20-tray Rational.
...
- **Primary button: "Apply now"** (provisional lock - confirm against the live site / SwipePages
...
- Route: ad → equipment-category or ICP landing page (SwipePages) → application. Mirror the
  landing-page headline in the hook where one exists.
