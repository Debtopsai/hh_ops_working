# HireHospo - brand constants for ad production

> **Sources of truth, in order:** (1) live brand assets in the working folder (a HireHospo UI kit,
> brand book, landing-page export) and the live site `hirehospo.com`; (2) the **business facts below,
> which are hard constants** drawn from the HireHospo operating skills (sales, credit, collections,
> ops) - do not vary them; (3) this file's visual section, which is **provisional** (no official UI
> kit exists yet) - if a kit or brand book appears, it wins and this file should be updated.
>
> **Portable build:** catalogue facts live in `references/catalogue.md` in this folder. If the
> `hirehospo-products` skill is installed alongside, it wins over that file.

**Contents:** 1 Business · 2 The offer · 3 Approved claims · 4 The 7 ICPs · 5 Awareness stages ·
6 Voice · 7 Banned messaging · 8 Credit + catalogue gates · 9 CTA · 10 Provisional visual system ·
11 Reusable ad frames · 12 File conventions
...
- **The enemy:** the capital hit. A fit-out as one giant cheque is daunting; the same fit-out as a
  ...
  gear outright, or draining working capital.
...
| Price bands | Category price bands from `references/catalogue.md` (e.g. glasswashers $2,300-$4,000) | Real bands only; ⚠-flag and verify before paid use |
...
margins, payment-day patterns) are **never** ad material.

## 4. The 7 ICPs (operator types) - angle + hero categories

Category facts and price bands come from `references/catalogue.md`. Specific product names and exact
prices are **not** in the portable build - look them up on `hirehospo.com` or ⚠-flag them. Match gear
to the audience - a cafe ad shows an undercounter dishwasher, not a 20-tray Rational.
...
Problem Unaware → Problem Aware (the quote-shock moment: "$18,000 for an oven?") → Solution Aware
(equipment finance exists) → Product Aware (HireHospo vs buying outright vs the bank) → Most Aware
(ready to apply). **No product in the Problem stages** - sell the felt cost of the capital hit;
HireHospo enters at the **bridge** (Solution Aware); the offer + "Apply now" land last (Most
Aware). The competitor in the Problem stages is **delay and outright purchase**, not a rival
financier - make the cost of tying up capital specific, stated flat.
...
(qualify and present, never chase). NZ English, plain numbers, restrained punctuation (exclamation
marks almost never). Reframes to reuse:

- **Daily-cost reframe:** "That's less than one flat white order a day" (only atop the approved
  "$4.66/day" figure - never fabricate new dailies).
...
- **Credit-led candour:** "We say no to plenty of deals. That's why the ones we say yes to work."
  (Use sparingly - it's a trust signal, not a boast.)
...
1. **The golden rule, ad edition:** no quote without credit approval - so no ad ever states a
   specific weekly or daily payment for a specific product. The sales team produces the figure
...
4. **Catalogue-true:** active products only; real brands, categories, price bands via
   `references/catalogue.md`; never invent specs (they live on the product page); link the product
   page where a product is named.
5. **Snapshot honesty:** catalogue counts and price bands are point-in-time - ⚠-flag any that will
   appear in paid creative and verify against the live site first.
6. **Fees stay internal:** late fees, admin fees, and deposit mechanics never appear in ads.

## 9. CTA

- **Primary button: "Apply now"** (provisional lock - confirm against the live site / the live
  landing pages; "Enquire now" is the acceptable softer variant). **One CTA per ad.**
- **Subline:** "Approved in 24 to 48 hours. Subject to credit approval."
- **The offer behind the CTA:** a credit-checked application → approval → quote → delivery in 1-3
  business days after deposit. The ad promises the *process*, never the outcome.
- Route: ad → equipment-category or ICP landing page → application. Mirror the landing-page
  headline in the hook where one exists.

## 10. Provisional visual system (no official kit yet - a kit/brand book in the folder WINS)
...
steel read premium, not discount).
...
- **Discipline:** **one flame highlight per frame**, on the key beat only. Photography of real
  catalogue equipment is encouraged (cut out on canvas or on a steel plinth); people optional and
  only for UGC/founder variants.
- **Motion:** transform + opacity only; settles ~0.5s ease-out; a "stamp" settle for APPROVED;
  respect `prefers-reduced-motion`.
- **Logo:** use the HireHospo wordmark asset from the live site. **Never redraw or invent a
  logomark.** If no asset is available, set "HireHospo" in the display face and ⚠-flag it in the
  hand-off. Wordmark at the bridge + end card only.

## 11. Reusable ad frames (build once, copy-tweak after)
...
| **Category grid** | 3-6 category tiles (real names from the catalogue) with price bands, one lit in flame. | For fit-out / multi-gear ads |
| **Capital-preservation ledger** | Mono ledger: "Capital kept in the business" vs "Oven in the kitchen" - both ticked. | Premium-gear ads (combi ovens) |
| **End card** | HireHospo wordmark + "Premium kitchen equipment, refurbished and warranted, on low weekly payments." + flame **Apply now** pill (accentink text) + mono subline "Approved in 24 to 48 hours · Subject to credit approval" + `hirehospo.com`. | The one place all required microcopy is guaranteed present |
...
- Canvas: **9:16 (1080×1920)** canonical; note 4:5 / 1:1 reflow in the storyboard.
